...
const ImportCSVforConcours = ({ onImportComplete, onClose }) => {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      setError(error.message || "Une erreur est survenue lors de l'importation du fichier");
      toast({
        title: "Erreur d'importation",
        description: error.message || "Une erreur est survenue lors de l'importation du fichier",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
        ...
        <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
          ...
          <p className="mb-2 text-sm text-gray-600 text-center">
            {isLoading ? (
              ...
              <span className="font-medium">Glissez-déposez votre fichier CSV ici</span>
            )}
          </p>
          <p className="text-xs text-gray-500 mb-4 text-center">
            {isLoading ? 'Veuillez patienter...' : 'ou cliquez pour sélectionner un fichier'}
          </p>
