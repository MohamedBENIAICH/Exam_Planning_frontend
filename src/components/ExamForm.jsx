...
const onFormSubmit = async (values) => {
  try {
    ...
    const classroomNames = values.classroom_ids
      .map(
        (id) => availableClassrooms.find((c) => c.id === id)?.nom_du_local || id
      )
      .join(", ");
    ...
    if (createdExam && createdExam.id) {
      console.log(
        "Starting exam scheduling process for exam ID:",
        createdExam.id
      );
      const schedulePromises = examData.classroom_ids.map(
        async (classroomId) => {
          const scheduleData = {
          ...
          console.log(
            `Scheduling exam for classroom ${classroomId} with data:`,
            scheduleData
          );
          try {
            await scheduleExam(scheduleData);
            console.log(
              `Successfully scheduled exam in classroom ${classroomId}`
            );
            toast.success(
              `Successfully scheduled exam in classroom ${classroomId}`
            );
            ...
            console.error(
              `Failed to schedule exam in classroom ${classroomId}:`,
              error
            );
            toast.error(
              `Failed to schedule exam in classroom ${classroomId}: ${error.message}`
            );
          }
        }
      );
      ...
      console.error(
        "Failed to get exam ID from creation response:",
        createdExam
      );
