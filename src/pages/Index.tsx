...
// Dummy chart component (replace with a real chart lib if you want)
const DonutChart = ({
  completed,
  upcoming,
}: {
  ...
  <div className="flex flex-col items-center">
    ...
    <div className="flex gap-2 mt-1">
      <span className="text-green-600 font-bold">{completed} Completed</span>
      <span className="text-blue-600 font-bold">{upcoming} Upcoming</span>
    </div>
  </div>
);
...
const ExamScheduling = () => {
  ...
  const handleAddEditExam = async (exam: Exam) => {
    try {
      ...
      if (editingExam) {
        ...
        const supervisorNames = exam.superviseurs
          .map((id) => {
      ...
      if (exams.length > 0) {
        ...
        console.log(
          ...
          exams.map((exam) => ({
            id: exam.id,
            module: exam.module,
            moduleIdType: typeof exam.module,
  ...
  const nextExam = exams.find((e) => e.status === "upcoming");
    ...
    <div className="min-h-screen flex bg-gray-50">
      ...
      <div className="flex-1 flex flex-col">
        <Header
          ...
          subtitle="Vue d'ensemble et prochaines étapes"
        />
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Card>
              ...
              <CardContent className="text-2xl font-bold">
                {
                  Array.from(new Set(exams.flatMap((e) => e.superviseurs)))
                    .length
                }
              </CardContent>
            </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="md:col-span-1 flex flex-col items-center justify-center">
              ...
              <CardContent>
                <DonutChart completed={completed} upcoming={upcoming} />
              </CardContent>
            </Card>
                ...
                {nextExam ? (
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold">{nextExam.module}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Clock className="text-gray-500" />
                      <span>
                        {new Date(nextExam.date).toLocaleString("fr-FR", {
                          weekday: "long",
                          year: "numeric",
                          month: "long",
                          day: "numeric",
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>{nextExam.classroom}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Users className="text-gray-500" />
                      <span>
                        {Array.isArray(nextExam.superviseurs)
                          ? nextExam.superviseurs.join(", ")
                          : ""}
                      </span>
                    </div>
                  </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Timeline des examens</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                {exams
                  ...
                  .map((exam, idx) => (
                    <li key={exam.id} className="mb-8 ml-4">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-semibold">{exam.module}</span>
                        ...
                        >
                          {exam.status === "completed" ? "Terminé" : "À venir"}
                        </span>
                      </div>
                      <div className="text-sm text-gray-500">
                        Surveillants:{" "}
                        {Array.isArray(exam.superviseurs)
                          ? exam.superviseurs.join(", ")
                          : ""}
                      </div>
                    </li>
                  ))}
                  ...
                  <div className="bg-gray-50 p-4 rounded-md">
                    ...
                    <div className="space-y-2">
                      {selectedExam.superviseurs &&
                        selectedExam.superviseurs.length > 0 && (
                          <div>
                            <span className="text-sm text-gray-600 font-medium">
                              Surveillants:
                            </span>
                            <p className="font-medium text-gray-800">
                              {getTeacherNames(selectedExam.superviseurs)}
                            </p>
                          </div>
                        )}
              ...
              <div className="pt-2">
                ...
                >
                  {showStudents ? (
                    ...
                    <>
                      <ChevronDown className="h-4 w-4" /> Afficher la liste des
                      étudiants ({selectedExam.students.length})
                    </>
                  )}
                </Button>
